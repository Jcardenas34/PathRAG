// This code should be placed in `app/page.tsx` of your Next.js project.
'use client';

import { useState, FormEvent, useRef, useEffect, ChangeEvent } from 'react';
import { Bot, User, SendHorizonal, Cpu } from 'lucide-react';
...
export default function ChatPage() {
  ...
  const llmOptions = ['gemini-2.5-flash', 'gemini-2.5-pro', 'gpt-4.1', 'gpt-o4-mini'];

  return (
    <div className="flex h-screen bg-gray-900 text-white font-sans">
      {/* Sidebar */}
      <div className="w-72 bg-gray-950 border-r border-gray-700 p-4 flex flex-col">
        <h1 className="text-xl font-bold mb-6">Propeterra RAG UI Prototype</h1>
      ...
      {/* Main Chat Area (No changes here) */}
          ...
          {messages.length === 0 ? (
            ...
            messages.map((message, index) => (
              <div key={index} className={`my-4 flex items-start gap-4 ${message.isUser ? 'justify-end' : ''}`}>
                ...
                <div className={`p-4 rounded-lg max-w-2xl ${message.isUser ? 'bg-indigo-600' : 'bg-gray-800'}`}>
                  <p>{message.text}</p>
                </div>
          ...
          <form onSubmit={handleSend} className="max-w-3xl mx-auto">
            <div className="relative">
              <textarea
                ...
                className="w-full bg-gray-800 border border-gray-600 rounded-lg p-4 pr-16 resize-none focus:outline-none focus:ring-2 focus:ring-indigo-500"
                rows={1}
              />
              <button
                type="submit"
                disabled={isLoading || !prompt.trim()}
                className="absolute right-4 top-1/2 -translate-y-1/2 p-2 rounded-lg bg-indigo-600 hover:bg-indigo-700 disabled:bg-gray-600 disabled:cursor-not-allowed transition-colors"
              >
